...
| `--changed-since=ref` | 按本地共同祖先筛选变更文件结果，不改变扫描范围和退出码 |
| `-h`, `--help` | 显示帮助 |
| `-v`, `--version` | 显示版本 |
...
`--changed-since=origin/main` 仍扫描全项目，只展示变更文件及其已知证据关联的结果；比较本地共同祖先与当前工作区，包含未被 Git 忽略的新文件，不拉取远程。仓库级和证据链截断的结果保留。所有格式披露隐藏数量；JSON 的 `changeView` 另含完整扫描统计。缺少本地引用、历史或 Git 时退出 `3`，不能与 `--baseline-write` 组合。此选项不是“只让新增问题阻断 CI”的策略。
...
支持 Supabase 构造器的命名导入别名，以及本地鉴权函数、返回函数式封装的命名导入、默认函数导入和重导出（最多 8 跳）。只在识别到对应调用位置时提供间接鉴权证据，降低置信度但保留结果；动态导入、复杂返回值、类型导入及未知封装不按鉴权成功处理。
...
- Supabase 策略及存储桶语句最多解析 4,000 个字符；超限标记扫描未完成。

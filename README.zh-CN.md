...
基线条目按规则、文件、标题以及原始来源内容的摘要匹配；只有没有可定位来源的结果才使用摘录。摘要在脱敏和截断之前计算，避免把替换后的新凭据当作旧问题。行号被刻意排除在外，因此在某条逐行结果上方编辑文件不会使其被报告为新增项。每个条目记录出现次数；超出该次数的额外出现会被报告。

基线格式现为第 2 版，SARIF 指纹名也升级为 `canshipFindingV2`。旧版基线会以退出码 `3` 被拒绝，不会自动转换或覆盖；请重新审阅结果后使用 `--baseline-write` 生成。
